...
- [Sustainability and Commercialization](#sustainability-and-commercialization)
...
| **Reduced child interaction** | **35% less time** spent on individualized interactions due to manual documentation demands |
...
| **Limited visibility** | Center directors lack real-time insight into pedagogical quality across classrooms |
...
PedaForge is a **12-month innovation project** proposed for the ECDA Early Childhood Innovation Sandbox. The platform integrates four interconnected modules that collectively reduce documentation burden, improve pedagogical quality, and equip center leaders with actionable data. Aligned with the refreshed Early Childhood Industry Digital Plan (IDP 2.0) announced at the Committee of Supply in March 2026, PedaForge demonstrates how responsible AI adoption can drive sector-wide transformation.

The project targets deployment across 2-3 pilot preschool centers in Singapore, with the goal of demonstrating measurable improvements in educator efficiency, teaching quality (as measured by QTT domains), and child developmental outcomes.
...
- **Differentiated scaffolding generation.** AI analyzes classroom profiles -- learning needs, developmental stages, learning styles, language abilities, and special considerations -- to produce activities with built-in accommodations for diverse learners.
...
- **AI-drafted developmental narratives.** After capturing raw observations, the AI drafts narrative summaries tagged to specific EYDF and NEL learning outcomes. Educators review and approve before narratives are finalized -- the AI assists, never replaces, professional judgment.
- **Living Profile engine.** Each child has a continuously updated developmental profile that synthesizes observations, assessment results, and portfolio entries into a holistic view. The Living Profile tracks progression across all EYDF domains over time.
...
- **Term summary report generation.** At the end of each term, the system synthesizes assessment checklists, observation records, and portfolio entries into a comprehensive summary report suitable for parent conferences and regulatory submissions.
...
- **QTT domain tagging.** Every coaching response includes metadata indicating which QTT domains are addressed, enabling aggregation and reporting at the center level.
- **Session persistence.** Coaching conversations are saved and analyzed over time to generate insights about an educator's growth trajectory, recurring themes, and areas of strength.
...
The dashboard gives center directors and pedagogical leaders a real-time, data-informed view of teaching quality across their center. It transforms coaching from a reactive, observation-dependent activity into a proactive, continuous process.

**Capabilities:**

- **Center-wide QTT domain averages.** Aggregated coaching data and assessment results visualized by QTT domain, showing center-level strengths and areas for focused improvement.
...
- **PLC topic recommendations.** The system analyzes center-wide patterns to suggest Professional Learning Community (PLC) discussion topics that address the most common pedagogical challenges.
...
- **Learning Needs Analysis (LNA) automation.** Generates center-level and individual LNA reports based on coaching data, QTT assessments, and SFw alignment, reducing the administrative burden of annual professional development planning.
...
- **Bi-annual IDP generation.** Drafts Individual Development Plans (IDPs) for each educator, pre-populated with evidence from coaching sessions, QTT assessments, and SFw mapping. Directors review and finalize collaboratively with each educator.
...
- **Role-based access control (RBAC)** enforced at the API layer. Educators see only their assigned cohorts. Directors see aggregated and anonymized views unless individually authorized.
...
| AI / LLM | Claude Opus 4 via Azure AI Foundry (Anthropic SDK) | Lesson generation, coaching, narrative drafting, alignment mapping |
| Embeddings | sentence-transformers (all-MiniLM-L6-v2) | Dense vector representations for semantic search |
| Vector Search | FAISS | Fast approximate nearest neighbor search over child documentation |
...
|-----------|---------------|
| **Zero Public AI Training** | All AI processing occurs within Azure AI Foundry's closed tenant environment. No child data is used to train public models. No data leaves the organization's Azure tenant. |
| **Singapore Data Residency** | All data -- at rest and in transit -- remains within Singapore-based Azure data centers. No cross-border data transfer. |
| **Role-Based Access Control** | Every user has a unique login. Educators access only their assigned cohorts. Directors access aggregated views with explicit authorization for individual educator data. |
...
| **Child Data Anonymization** | Any data processed outside the primary database (e.g., embedding generation) is anonymized. Child identifiers are replaced with opaque tokens. |
...
|-----------|---------------|-------------|
| Child observations and portfolios | Confidential | Assigned educators, authorized directors |
| Coaching session transcripts | Private | Individual educator, authorized directors |
| QTT assessment scores | Internal | Educator (own), directors (aggregated + individual with authorization) |
| Center-level analytics | Internal | Directors, pedagogical leaders |
...
PedaForge is grounded in established educational research and evidence-based pedagogical frameworks. The platform does not invent new pedagogy -- it operationalizes proven approaches through technology.
...
|--------|----------|--------|-------------------|--------|
| Documentation time per educator | 8 hrs/week | 3 hrs/week (60% reduction) | Pre/post time-tracking surveys, system usage logs | Months 5-9 |
...
| IDP accuracy rating | Subjective annual review | 85% educator agreement with AI-generated IDP | Post-assessment surveys and educator interviews | Months 10-11 |
| Coaching engagement | Ad hoc, observation-dependent | 2+ sessions per educator per week | System usage analytics | Months 5-9 |
| QTT score improvement | Pre-pilot baseline assessment | +0.5 points average across QTT domains | Pre/post QTT assessment by trained assessors | Months 5, 9 |
| Educator retention intent | Industry benchmark | 90% of pilot participants would continue using the platform | End-of-pilot survey | Month 10 |

### Secondary Metrics

- Parent engagement rate with AI-generated home activity suggestions
- Portfolio completeness (observations per child per week)
- Director dashboard login frequency and feature utilization
...
### Phase 1: Architecture and AI Foundation (Months 1-2)
...
- Coaching engine v1: Composable prompt architecture, Reflective Practice mode
- Database schema design and migration framework
- PDPA compliance review and data classification

### Phase 2: Platform Build (Months 3-4)

- Lesson planner module: profile ingestion, theme-based generation, differentiation engine
- Portfolio module: observation capture, narrative drafting, Living Profile engine
- Coaching engine v2: All four coaching modes, anti-sycophancy layer, safety guardrails
- Director dashboard: QTT aggregation, educator profiles, SFw mapping
- Frontend development: responsive design, SSE streaming integration, Chart.js visualizations
- Integration testing across all modules
- Security audit and penetration testing

### Phase 3: Pilot Deployment (Month 5)

- Partner center onboarding (2-3 preschool centers)
- Educator training workshops (2 sessions per center)
- Director orientation and dashboard training
- Pre-pilot QTT baseline assessment
- Pre-pilot documentation time survey
- System monitoring and rapid iteration based on early feedback

### Phase 4: Active Classroom Execution (Months 6-9)
...
### Phase 6: Final Reporting and Scale Plan (Month 12)

- Final impact report with statistical analysis
- Presentation to ECDA and stakeholders
- Sustainability and commercialization roadmap
- Technical documentation for handoff or scaling
- Lessons learned and recommendations for IDP 2.0 program
...
| Phase | Component | Cost (SGD) |
|-------|-----------|-----------|
| **Architecture and AI** | Cloud infrastructure (Azure App Service, PostgreSQL, Key Vault) | $18,000 |
| | AI API costs (Claude Opus 4 via Azure AI Foundry, 12 months) | $24,000 |
| | Full-stack development (FastAPI backend, React/Next.js frontend) | $18,000 |
| | Embedding pipeline and vector search infrastructure | $6,000 |
| | QTT and SFw alignment engine development | $6,000 |
| | **Subtotal** | **$72,000** |
| **Security and Compliance** | PDPA compliance audit and implementation | $8,000 |
| | Penetration testing and security hardening | $4,000 |
| | Data encryption, access control, audit logging | $4,000 |
| | **Subtotal** | **$16,000** |
| **Change Management** | Educator training program development and delivery | $8,000 |
| | Director onboarding and dashboard training | $4,000 |
| | User documentation and support materials | $3,000 |
| | Ongoing user support during pilot (Months 5-9) | $3,000 |
| | **Subtotal** | **$18,000** |
| **Project Management** | Project coordination and stakeholder management | $8,000 |
| | Impact assessment and data analysis | $6,000 |
| | Final reporting, presentation, and scale planning | $5,000 |
| | **Subtotal** | **$19,000** |

| | |
|---|---|
| **Total Project Budget** | **SGD $125,000** |

### Budget Notes

- All costs are in Singapore Dollars (SGD).
- Cloud infrastructure costs assume Azure App Service B2 tier with PostgreSQL Flexible Server, scaled for 2-3 pilot centers.
- AI API costs are estimated based on projected usage of approximately 500 coaching sessions per month, 200 lesson plans per month, and 1,000 portfolio narratives per month across all pilot centers.
...
|------|-----|---------------------|
| **Project Lead** | 1.0 | Overall project timeline and budget management. Stakeholder communication with ECDA, pilot centers, and institutional partners. Risk management and escalation. |
| **Technical Architect** | 1.0 | Full-stack development (FastAPI, React/Next.js). System architecture and infrastructure. Database design. Security implementation. CI/CD pipeline. |
| **AI Engineer** | 1.0 | Prompt engineering and coaching engine development. QTT alignment mapping. Anti-sycophancy research and implementation. Embedding pipeline and RAG architecture. |
| **Pedagogical Lead** | 0.5 | EYDF, NEL, QTT, and SFw domain expertise. Validation of AI-generated content against curriculum standards. Coaching mode design and quality assurance. Educator training facilitation. |
| **UI/UX Designer** | 0.5 | Interface design for educators and directors. Usability testing with pilot participants. Responsive design for classroom use (tablet-first). Accessibility compliance. |
| **Impact Evaluator** | 0.3 | Pre/post assessment design and administration. Quantitative and qualitative data analysis. Impact report authoring. Statistical methodology and rigor. |
...
- Preschool operators and center directors for practical feasibility review

---

## Framework Alignment

PedaForge is designed to align with and operationalize multiple national frameworks governing early childhood education in Singapore.
...
The QTT is the primary rubric framework for the coaching agent. All four coaching modes reference QTT domains and rubric descriptors. The director dashboard aggregates QTT-tagged coaching data to provide center-level quality insights.
...
PedaForge connects individual educator development to broader national skills development objectives, ensuring that professional growth within the platform contributes to recognized national qualifications and career progression pathways.
...
| `dashboard.html` | Director's Leadership Dashboard with QTT visualizations, educator profiles, and PLC recommendations |
...
1. **SaaS subscription model.** Monthly or annual subscription pricing for preschool operators, tiered by center size and number of educators. Pricing designed to be accessible for both large chains and independent operators.
...
4. **ASEAN market expansion.** The platform architecture supports localization for other ASEAN markets with similar ECE quality frameworks. Initial expansion targets include Malaysia, Thailand, and the Philippines, where workforce quality and documentation challenges mirror Singapore's experience.

### Long-Term Vision

- Longitudinal child development analytics spanning multiple years
- Cross-center benchmarking for operators managing multiple sites
