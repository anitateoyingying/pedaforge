...
|                       FastAPI Backend                              |
|  +---------------+  +----------------+  +----------------------+  |
|  | API (Swagger) |  |  AI Services   |  |    Data Layer         |  |
|  |               |  |                |  |                      |  |
...
|  |  Entra ID Auth|  |  SFw Aligner   |  |     Coach Sessions)  |  |
|  +---------------+  +----------------+  +----------------------+  |
|                                                                    |
|  PDPA Compliant | Entra ID + Role-Based Access | Audit Logging    |
+------------------------------------------------------------------+
```
...
- **Swagger/OpenAPI documentation** auto-generated by FastAPI at `/docs`, providing interactive API testing and a complete reference for all endpoints.
...
- **Role-based access control (RBAC)** enforced at the API layer via Microsoft Entra ID. Educators see only their assigned cohorts. Directors see aggregated and anonymised views unless individually authorised.
...
| Authentication | Microsoft Entra ID, Entra External ID, role-based access control | Secure educator and director authentication with enterprise SSO |
...
| | **Subtotal** | **$10,820** |
| **Security and Compliance** | PDPA compliance review (internal IT department) | $2,000 |
| | Security assessment (internal IT department) | $1,200 |
| | **Subtotal** | **$3,200** |
...
| `AZURE_ENTRA_TENANT_ID` | Microsoft Entra ID tenant identifier |
| `AZURE_ENTRA_CLIENT_ID` | Microsoft Entra ID application (client) identifier |
